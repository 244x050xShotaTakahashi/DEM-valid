...
integer(kind=4) :: i,j,k,i1,j1,k1,m,n,l,sind,eind,ns,N,stride,dnum,c
...
real(kind=8) :: vxt,vyt,vzt,q,qm,r,v0,g,conve,convj,pi,dm,dv,ds,dq,den,theta,distance,distance2,e,bar
...
open(11,file='output.dat', status='replace')
      ...
      write(20,"('POINTS ',i5,' float')") dnum
      do m=ns,dnum
         write(20,"(3(f10.4,1x))") pbuf(m)%x, pbuf(m)%y, pbuf(m)%z
      end do
      write(20,"('CELLS ',i5,'  ',i5)") dnum, dnum*2
      do m=ns,dnum
         write(20,"('1 ',i)") m-1
      end do
      write(20,"('CELL_TYPES ',i5)") dnum
      do m=ns,dnum
         write(20,"('1')")
      end do
   
      write(20,"('POINT_DATA ',i5)") dnum
     ...
     &    + wrk(J3Z,i1,j1,k )*v7 + wrk(J3Z,i ,j1,k )*v8
      ...
      if(pbuf(m)%nid == -1) then
         do l = m,dnum-1
            ...
            pbuf(m)%y=pbuf(m+1)%y
            pbuf(m)%z=pbuf(m+1)%z
            pbuf(m)%vx=pbuf(m+1)%vx
            pbuf(m)%vy=pbuf(m+1)%vy
            pbuf(m)%vz=pbuf(m+1)%vz
            pbuf(m)%q=pbuf(m+1)%q
            pbuf(m)%nid = pbuf(m+1)%nid
         end do
         dnum = dnum-1
      end if
