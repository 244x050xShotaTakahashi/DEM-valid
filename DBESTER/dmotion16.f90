program dmotion16
  ...
  integer(kind=4) :: stvtk,stdat,stnden,lavtk,ladat,nx,ny,nz,jmode,oml,dat,vtk,nden,reflect,fix,periodic,xyfix,holeonly
  ...
  namelist /jobcon/ nstep,dat,vtk,nden,oml,jmode,reflect,fix,periodic,xyfix,holeonly
  ...
  open(18,file='output.dat',status='replace')
  ...
  !open(12,file='lunarsurface.dat', status='replace')
  !open(13,file='bottomsurface.dat', status='replace')
  do k=0,zu-zl
     do j=0,yu-yl
        do i=0,xu-xl
           distance  = ((i-xholec)**2+(j-yholec)**2)**0.5
           if(rhole<=distance.and.lbhole<k.and.k<ubhole) then
              wrk(EX,i,j,k) = 0
              wrk(EY,i,j,k) = 0
              wrk(EZ,i,j,k) = 0
           else if(rhole<distance.and.k==ubhole) then
              wrk(EZ,i,j,k) = 2*wrk(EZ,i,j,k+1) - wrk(EZ,i,j,k+2)
              wrk(EZ,i,j,k-1) = 2*wrk(EZ,i,j,k) - wrk(EZ,i,j,k+1)
              !write(12,*) i,j,k,wrk(EZ,i,j,k),wrk(EZ,i,j,k-1),wrk(EZ,i,j,k+1),wrk(EZ,i,j,k+2)
           else if(distance<rhole.and.k==lbhole) then
              wrk(EZ,i,j,k) = 2*wrk(EZ,i,j,k+1) - wrk(EZ,i,j,k+2)              
              wrk(EZ,i,j,k-1) = 2*wrk(EZ,i,j,k) - wrk(EZ,i,j,k+1)
              !write(13,*) i,j,k,wrk(EZ,i,j,k-1),wrk(EZ,i,j,k),wrk(EZ,i,j,k+1),wrk(EZ,i,j,k+2)
           end if
        end do
     end do
  end do
  ...
  do n = 1,nstep
     ...
     do m=ns,dnum
        ...
        if(oml.eq.1) then
           !----------- caluculate charge
           !upper hole
           if(rhole < distance.and.pbuf(m)%z>=ubhole) then
              if(0<Vd) then
              ...
              if(rhole < distance2) then
                 if(0<Vd) then
                    ...
                    !write(18,*) 'under_shade',' q',q,'jj2',jj2,'jj2_oml',jj2*exp(-(e*Vd)/(bk*Ti)),'-jj1',-jj1,'-jj1_oml',-jj1*(1+(e*Vd)/(bk*Te)),'-jj3',-jj3,'-jj3_oml',-jj3*(1+(e*Vd)/(bk*Tph)),'x',pbuf(m)%x,'y',pbuf(m)%y,'z',pbuf(m)%z,'distance',distance
                 else
        ...
        if(q < 0) then
           ...
           !if(distance<rhole+0.1.and.rhole-0.1<distance) write(18,*) 'shade',' distance',distance,'x',pbuf(m)%x,'y',pbuf(m)%y,'z',pbuf(m)%z,'Jnet',q/dt
        else  
           random2(m) = random2(m)*e
           !if(distance<rhole+0.1.and.rhole-0.1<distance) write(18,*) 'sun',' distance',distance,'x',pbuf(m)%x,'y',pbuf(m)%y,'z',pbuf(m)%z,'Jnet',q/dt
        ...
        if(xyfix.eq.1) then
           ...
           if(.not.(distance<rhole+0.01.and.rhole-0.01<distance.and.pbuf(m)%z<ubhole)) then
              pbuf(m)%vz = pbuf(m)%vz + (eez*pbuf(m)%q*e/dm)*dt - g*dt
           end if
        end if
        ...
        if(rhole<distance.and.rhole<distanceb.and.zb<ubhole) then
           pbuf(m)%x = xb
           pbuf(m)%y = yb
        end if
        ...
        if(fix.eq.1) then
           ...
           if(pbuf(m)%z<lbhole.and.distance<rhole) then
              ...
              pbuf(m)%z = lbhole
              distance = ((pbuf(m)%x-xholec)**2+(pbuf(m)%y-yholec)**2)**0.5
           else if(pbuf(m)%z<ubhole.and.rhole<distance.and.ubhole<=zb) then
        ...
        if(pbuf(m)%x<xl.or.xu<pbuf(m)%x.or.pbuf(m)%y<yl.or.yu<pbuf(m)%y.or.zu<pbuf(m)%z.or.pbuf(m)%z<lbhole.or.(fix.ne.1.and.pbuf(m)%z<ubhole)) pbuf(m)%nid = -1
