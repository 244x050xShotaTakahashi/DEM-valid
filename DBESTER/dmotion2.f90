...
character(len=40) :: fnorgx, fnorgy, fnorgz, fnorgx2, fnorgy2, fnorgz2, fnorg
...
real(kind=8) :: vxt,vyt,vzt,q,qm,r,g,conve,convj,pi,dm,dv,ds,dq,den,theta,distance,distance2,a,e
...
write(fnorg,'(a,i3.3,a,i3.3,a)') 'dq00000000_id:', 0, '-', N/stride ,'.h5'
...
do n = 0,N
   do m=ns,ne
     ...
     &    + wrk(J3Z,i1,j1,k )*v7 + wrk(J3Z,i ,j1,k )*v8
      ...
      jj1 = (jj1x**2+jj1y**2+jj1z**2)**0.5*dv
      jj2 = (jj2x**2+jj2y**2+jj2z**2)**0.5*dv
      jj3 = (jj3x**2+jj3y**2+jj3z**2)**0.5*dv
      ...
      !    jplus = jjph + jj2
      !    jminus = jj1 + jj3
      !    if(jplus < jminus) then
      !       dq = (jminus - jplus)*dt
      !       if(a*e < dq) pbuf(m)%q = pbuf(m)%q - e
      !    else
      !       dq = (jplus - jminus)*dt
      !       if(a*e < dq) pbuf(m)%q = pbuf(m)%q + e
      !    end if
      ...
      write(11,'(i5,3f12.6)') n, pbuf(m)%x, pbuf(m)%y, pbuf(m)%z 
   end do
   ...
   if(mod(n,stride)==0) then
      ...
      write(sdsname,'(a,i4.4)') fnorg(1:2),n/stride
      ...
      call wrt1d(idt7,sdsname,ne,pbuf(ns:ne)%q,ist1,ist2)
   end if
