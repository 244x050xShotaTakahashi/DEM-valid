  ...
  do n = 1,nstep
     ...
     do m=ns,dnum
        ...
        if(xyfix.eq.1) then
           ...
           if(.not.(distance<rhole+0.00001.and.rhole-0.00001<distance.and.pbuf(m)%z<ubhole)) then
              pbuf(m)%vz = pbuf(m)%vz + (eez*pbuf(m)%q*e/dm)*dt - g*dt
           end if
        end if
        ...
        if(pbuf(m)%x<xl.or.xu<pbuf(m)%x.or.pbuf(m)%y<yl.or.yu<pbuf(m)%y.or.zu<pbuf(m)%z.or.pbuf(m)%z<lbhole) pbuf(m)%nid = -1
