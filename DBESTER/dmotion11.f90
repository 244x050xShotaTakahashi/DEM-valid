...
integer(kind=4) :: i,j,k,i1,j1,k1,m,n,l,sind,eind,ns,N,stride,dnum,cc
...
real(kind=8),allocatable :: ej(:,:,:,:),wrk(:,:,:,:),random(:,:),random2(:)
...
parameter(bk=1.38064852-23)
parameter(Te=8.6*1.60217662E-19)
parameter(Ti=8.6*1.60217662E-19)
parameter(Tph=2.2*1.60217662E-19)
parameter(g=1.6)
...
!$omp parallel default(private) shared(N,ns,dnum,theta,ubhole,xholec,yholec,lbhole,ds,dt,r,rhole,jjph,dm,zu,xl,xu,yu,yl,pbuf,wrk,stride,iind)
do n = 1,N
  ...
  time0=omp_get_wtime()
     ...
     &    + wrk(J3Z,i1,j1,k )*v7 + wrk(J3Z,i ,j1,k )*v8
      ...
      if(rhole < distance) then
         if(0<Vd) then
            ...
            !write(11,*) '1',q,exp(-(e*Vd)/(bk*Tph))*(1+(e*Vd)/(bk*Tph)),(e*Vd)/(bk*Tph),exp(-(e*Vd)/(bk*Ti)),(1+(e*Vd)/(bk*Te))
         else
      ...
      if(lbhole<pbuf(m)%z.and.pbuf(m)%z<ubhole.and.rhole<distance.and.distanceb<rhole) then
         ...
         write(11,*) pbuf(m)%x,pbuf(m)%y,distance
   ...
   !----------- output dust data
   if(mod(n,stride)==0.and.dnum/=0) then
   ...
   write(6,*) n,' ',time1-time0
end do
!$omp end parallel

close(11)

end program dmotion11
