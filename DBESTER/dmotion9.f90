...
real(kind=8) :: eex,eey,eez,jj1,jj1x,jj1y,jj1z,jj2,jj2x,jj2y,jj2z,jj3,jj3x,jj3y,jj3z,jph,jjph,jplus,jminus,Vd,Cd,e0
   ...
   do m=ns,dnum
     ...
     &    + wrk(J3Z,i1,j1,k )*v7 + wrk(J3Z,i ,j1,k )*v8
      ...
      if(lbhole<pbuf(m)%z.and.pbuf(m)%z<ubhole.and.rhole<distance.and.distanceb<rhole) then
         ...
         pbuf(m)%vx = cos(theta3)*pbuf(m)%vx-sin(theta3)*pbuf(m)%vy+intx-intx*cos(theta3)+inty*sin(theta3)
         pbuf(m)%vy = sin(theta3)*pbuf(m)%vx+cos(theta3)*pbuf(m)%vy+inty-intx*sin(theta3)-inty*cos(theta3)
